...
_asm { \
	...
	_asm push _SELECTOR_KER_DS		/* 首先将堆栈段选择符(SS)入栈。*/\
	_asm push eax					/* 然后将保存的堆栈指针值(esp)入栈。*/\
	_asm pushfd						/* 将标志寄存器(eflags)内容入栈。*/\
	_asm push _SELECTOR_KER_CS		/* 将内核代码段选择符(cs)入栈。*/\
	_asm push offset l1				/* 将下面标号l1 的偏移地址(eip)入栈。*/\
	_asm iretd						/* 执行中断返回指令，则会跳转到下面标号l1 处。*/\
_asm l1: mov eax,_SELECTOR_KER_DS	/* 此时开始执行任务0，*/\
